...
from inceptionnext import models
import timm
...
class EDViTED(nn.Module):
    def __init__(self, config, pretrained=True):
        super(EDViTED, self).__init__()
        self.encoder = Encoder()
        self.decoder = Decoder()
        self.backbone = models.inceptionnext_small(pretrained=pretrained)
        self.embedder = timm.create_model(
            config["model"]["embedder"], pretrained=pretrained
        )
        self.backbone.patch_embed = HybridEmbed(
            self.embedder, img_size=config["img_size"], embed_dim=768
        )

        self.num_features = self.backbone.head.fc2.out_features * 2
