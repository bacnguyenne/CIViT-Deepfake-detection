...
from model.edvit_ed import EDViTED
from model.edvit_vae import EDViTVAE
...
def train_model(
    ...
    if mod == "ed":
        ...
        model = EDViTED(config)
    else:
        from train.train_vae import train, valid
        model = EDViTVAE(config)
    ...
    file_path = os.path.join(
        "weight",
        f'EDvit_{mod}_{time.strftime("%b_%d_%Y_%H_%M_%S", time.localtime())}',
    )
...
def gen_parser():
    parser = optparse.OptionParser("Train EDViT model.")
    parser.add_option(
        "-e",
        "--epoch",
        type=int,
        dest="epoch",
        help="Number of epochs used for training the EDvNextViT model.",
    )
    ...
    parser.add_option(
        ...
        "--model",
        dest="model",
        help="model ed or model vae, model variant: EDvit (A) ed or EDvit (B) vae.",
    )
    parser.add_option(
        "-p",
        "--pretrained",
